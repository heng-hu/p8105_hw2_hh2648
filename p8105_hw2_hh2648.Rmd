...
The dataset `full_rent` has ``r ncol(full_rent)`` variables and ``r nrow(full_rent)`` observations. It contains rental prices in New York City from ``r paste(full_rent[1, "rent_year"], full_rent[1, "rent_month"], sep = "/")`` to ``r paste(full_rent[nrow(full_rent), "rent_year"], full_rent[nrow(full_rent), "rent_month"], sep = "/")``. It contains ``r length((unique(full_rent$zip_code)))`` unique ZIP codes and ``r length((unique(full_rent$neighborhoods)))`` unique neighborhoods.
...
```
The dataset `zip_wo_rent` was created to include all the ZIP codes without any rental observations. After examining `neighborhood` count numbers in this dataset, we found most of them don't have associated `neighborhood` information, suggesting they may be reserved for future data expansion.
...
The dataset `rent_drop` was create to compare rental prices in January 2021 to prices in January 2020 for all available ZIP code. It contains ``r nrow(rent_drop)`` records with valid rental price difference between the two time points. The variable `rent_drop` stores the calculated difference, while `rent_rank` represents the rank of rental prices in January 2020.

The dataset `rent_drop_10` lists the 10 ZIP codes with largest drop in price from January 2020 to 2021. All of them are from ``r unique(rent_drop_10$county)`` County. Most of them also ranked among the top rental prices in January 2020 base on `rent_rank`.